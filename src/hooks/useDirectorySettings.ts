...
import { homeDir, isWebRuntime, join } from "@/lib/runtime";
...
export function useDirectorySettings({
  ...
  useEffect(() => {
    ...
    // WebUI 不展示或操作宿主机目录。这里必须在任何路径 RPC 前短路，
    // 否则即使目录设置界面已隐藏，打开设置页仍会产生桌面命令错误。
    if (isWebRuntime()) {
      setAppConfigDir(undefined);
      initialAppConfigDirRef.current = undefined;
      setIsLoading(false);
      return () => {
        active = false;
      };
