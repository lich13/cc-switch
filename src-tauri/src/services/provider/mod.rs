...
/// Codex takeover is route-only, so selecting any Codex provider only updates
/// the database route and never takes ownership of the live Codex files.
/// Other applications retain the existing official-provider block.
pub fn official_provider_supports_proxy_takeover(app_type: &AppType, _provider: &Provider) -> bool {
...
"#;
    ...
    #[test]
    #[serial]
    fn update_current_codex_provider_catalog_is_db_only() {
        let _home = TempHome::new();
        crate::settings::reload_settings().expect("reload settings");
        ...
        updated.settings_config["config"] =
        ...
        ProviderService::update(&state, AppType::Codex, None, updated)
            .expect("catalog update should remain database-only");
        ...
        let saved = db
            ...
            .expect("provider exists");
        assert_eq!(
            saved.settings_config["modelCatalog"]["models"][0]["model"],
            "gpt-5.4"
        );
        assert_eq!(fs::read(&auth_path).expect("read auth.json"), original_auth);
        assert_eq!(
        ...
        assert!(
            ...
            "RouteOnly updates must not generate a live model catalog"
        );
    }
    ...
    #[test]
    #[serial]
    #[ignore = "Codex RouteOnly 不清理或回填 live OAuth 凭据"]
    ...
    #[test]
    #[serial]
    #[ignore = "Codex RouteOnly 不从 live auth.json 采纳 CLI refresh token"]
...
"#
    ...
    #[test]
    #[serial]
    #[ignore = "Codex RouteOnly 不比较或覆盖 live refresh token"]
    ...
    #[test]
    #[serial]
    #[ignore = "Codex RouteOnly 不读取或删除 legacy live refresh token"]
    ...
    #[test]
    #[serial]
    fn codex_auth_center_remove_and_logout_preserve_live_files() {
        with_test_home(|state, _| {
            crate::settings::reload_settings().expect("reload settings");
            tauri::async_runtime::block_on(async {
                state
            ...
            fs::write(
                crate::codex_config::get_codex_auth_path(),
                br#"{"auth_mode":"chatgpt","tokens":{"access_token":"native-access"}}"#,
            )
            .expect("seed native auth.json");
            fs::write(
                crate::codex_config::get_codex_config_path(),
                b"model = \"gpt-5.4\"\n",
            )
            .expect("seed native config.toml");
            let live_before = crate::codex_config::CodexLiveStateSnapshot::capture()
                .expect("capture native Codex live state");
            ProviderService::switch(state, AppType::Codex, &provider.id)
                .expect("activate managed provider");
            assert_eq!(
                crate::codex_config::CodexLiveStateSnapshot::capture()
                    .expect("capture live after managed route switch"),
                live_before
            );
            ...
            assert_eq!(
                crate::codex_config::CodexLiveStateSnapshot::capture()
                    .expect("capture live after account removal"),
                live_before,
                "Auth Center removal must not mutate Codex live files"
            );
            assert_eq!(
                state
                    .db
            ...
            assert_eq!(
                crate::codex_config::CodexLiveStateSnapshot::capture()
                    .expect("capture live after account reactivation"),
                live_before
            );

            tauri::async_runtime::block_on(state.managed_auth.codex_oauth.clear_auth())
                .expect("logout all managed accounts");
            assert!(
                tauri::async_runtime::block_on(state.managed_auth.codex_oauth.list_accounts())
                    .is_empty()
            );
            assert_eq!(
                crate::codex_config::CodexLiveStateSnapshot::capture()
                    .expect("capture live after managed logout"),
                live_before,
                "Auth Center logout must not mutate Codex live files"
            );
        });
    }
        ...
        with_test_home(|state, _| {
            ...
            assert_eq!(
                crate::codex_config::CodexLiveStateSnapshot::capture()
                    .expect("capture live after managed logout"),
                live_before,
                "locked logout must preserve Codex live files"
            );
        });
    }
    ...
    #[test]
    #[serial]
    #[ignore = "Codex RouteOnly 的 current 回滚由数据库路由测试覆盖，不创建 live bundle"]
    ...
    #[test]
    #[serial]
    #[ignore = "Codex RouteOnly 不创建或更新 Codex live backup"]
    ...
    #[test]
    #[serial]
    #[ignore = "Codex RouteOnly 更新不重建当前 provider 的 live auth"]
...
impl ProviderService {
    ...
    fn preflight_managed_codex_auth(
        state: &AppState,
        app_type: &AppType,
        provider: &Provider,
    ) -> Result<(), AppError> {
            ...
            && Self::managed_codex_oauth_account_id(provider).is_some()
        {
            build_effective_provider_for_live_with_codex_oauth_manager(
                ...
                app_type,
                provider,
                &state.managed_auth.codex_oauth,
            )?;
        }
        Ok(())
    }
    ...
    pub fn add(
        ...
        let _codex_add_guard = if matches!(app_type, AppType::Codex) {
        ...
        if is_managed_codex_add {
            ...
            Self::preflight_managed_codex_auth(state, &app_type, &provider)?;
            ...
            {
                let rollback = match previous_provider.as_ref() {
                ...
                return match rollback {
                    ...
                    Err(rollback_error) => Err(AppError::Message(format!(
                        "新增首个托管 Codex provider 失败: {error}; 回滚 Provider 数据同时失败: {rollback_error}"
                    ))),
                };
            }
            return Ok(true);
    ...
    fn switch_codex_route_only(
        ...
        Self::preflight_managed_codex_auth(state, &AppType::Codex, provider)?;
        ...
        .map_err(|e| AppError::Message(format!("切换 Codex 本地路由失败: {e}")))?;
    ...
    fn switch_normal(
        ...
        if matches!(app_type, AppType::Codex) {
            return Self::switch_codex_route_only(state, id, provider);
        }
