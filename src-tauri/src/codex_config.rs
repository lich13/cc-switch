...
fn plan_codex_live_write(
    ...
    build_codex_live_write_plan(category, auth, config_text, preserve_official_login)
}

fn build_codex_live_write_plan(
    category: Option<&str>,
    auth: &Value,
    config_text: Option<&str>,
    preserve_official_login: bool,
...
"#;
    ...
    #[test]
    #[serial]
    #[ignore = "Codex RouteOnly 不创建或迁移 live OAuth marker"]
    ...
    #[test]
    #[serial]
    #[ignore = "Codex RouteOnly 不读取或迁移 live OAuth marker"]
    fn legacy_managed_marker_migrates_by_user_without_breaking_refresh_rollback() {
        ...
        assert_eq!(migrated.id_token.as_deref(), Some(id_token.as_str()));
        assert!(migrated.last_refresh_ms.is_some());
        assert_eq!(migrated.chatgpt_account_id, "legacy-workspace");
        assert!(codex_live_auth_is_managed_chatgpt_login(
            &auth_r0,
            "legacy-workspace"
        ));
    ...
    #[test]
    #[serial]
    #[ignore = "Codex RouteOnly 不删除 live OAuth 凭据或 marker"]
...
base_url = "https://bedrock.example/v1"
"#;
        let plan = build_codex_live_write_plan(Some("official"), &json!({}), Some(config), false)
    ...
    fn third_party_plan_stamps_requires_openai_auth_to_match_preservation() {
        ...
        let off = build_codex_live_write_plan(None, &auth, Some(stale_true), false)
        ...
        let on = build_codex_live_write_plan(None, &auth, Some(stale_true), true)
        ...
        let on_flagless = build_codex_live_write_plan(None, &auth, Some(flagless), true)
    ...
    fn requires_openai_auth_stamp_only_touches_tables_with_a_request_auth_short_circuit() {
        ...
        for preserve in [false, true] {
            let plan = build_codex_live_write_plan(None, &json!({}), Some(header_auth), preserve)
        ...
        let plan = build_codex_live_write_plan(None, &json!({}), Some(env_key), false)
    ...
    fn preflight_rejects_provider_table_conflicts_codex_refuses_to_load() {
        ...
        for config in rejected {
            assert!(
                build_codex_live_write_plan(None, &with_key, Some(config), false).is_err(),
                "third-party preflight must refuse:\n{config}"
            );
            assert!(
                build_codex_live_write_plan(Some("official"), &json!({}), Some(config), false)
                    .is_err(),
        ...
        for config in accepted {
            assert!(
                build_codex_live_write_plan(None, &json!({}), Some(config), false).is_ok(),
